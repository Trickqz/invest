                                ...
                                if (typeof page__extract_unilevel__buttons === 'undefined') {
                                    ...
                                    for(let i = 0; i<page__extract_unilevel__buttons.length; i++){
                                        page__extract_unilevel__buttons[i].addEventListener('click', () => {
                                            ...
                                            fc_incomeGetAll(page__extract_unilevel__buttons[i].getAttribute("data-tab"));
                                        });
                                    }
                                }
                            </script>
...
<script>
    var elDate_of = document.getElementById("income_date_of");
    var elDate_until = document.getElementById("income_date_until");
    ...
    function fc_incomeGetAll(status = "", listType){
        ...
        $.ajax({
            type: 'GET',
            url: CFG__API_URL + CFG__API_VERSION + "/income?date_of="+FormatStringDate(elDate_of.value)+"&date_until="+FormatStringDate(elDate_until.value),
            data: {},
                    ...
                    if (arr.length > 0) {
                        for (var i = 0; i < arr.length; i++){
                            ...
                            buttons += 
                                ...
                                + '        <a class="dropdown-item text-danger" href="javascript:;" onclick="fc_incomeDelete(\'' + arr[i]._id + '\');">'
                                + '            Excluir'
                                + '        </a>'
                                + '    </div>'
                                + '</span>';*/
                            
                            var variationIncome = arr[i]._bid - arr[i]._open,
                                variationIncomeStr = '';
                            ...
                            tbodyHTML += 
                                ...
                                + '        </div>'
                                + '    </td>'
                                //+ '    <td class="text-muted text-center" data-label="Previous">'
                                //+ '        ' + open
                                //+ '    </td>'
                                + '    <td class="text-muted text-center" data-label="Current">'
                                + '        ' + bid + "%"
                                + '    </td>'
                                + '    <td class="text-muted text-center" data-label="Variation">'
                                + '        ' + variationIncomeStr
                                + '    </td>'
                                + '    <td>'
                                ...
                                //+ '                 Editar'
                                //+ '             </button>'
                                + '             <button type="button" class="btn btn-danger" onclick="fc_incomeDelete(\'' + arr[i]._id + '\');">'
                                + '                 <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-pencil" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">'
                                + '                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>'
                                + '                    <path d="M4 20h4l10.5 -10.5a1.5 1.5 0 0 0 -4 -4l-10.5 10.5v4"></path>'
                                + '                    <line x1="13.5" y1="6.5" x2="17.5" y2="10.5"></line>'
    ...
    function fc_incomeDelete(_id) {
        alertOpen({
                title: "Confirm",
                text: "Delte quotas?<br /><b>Warning:</b> The quote will be permanently deleted!",
                type: "danger",
            ...
            function(){
				...
				$.ajax({
                    type: 'DELETE',
                    url: CFG__API_URL + CFG__API_VERSION + "/income/"+_id,
                    beforeSend: function (xhr){ 
                        xhr.setRequestHeader('Authorization', cookieGet('__utmo'));
                    },
                    data: {},
