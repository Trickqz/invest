                ...
                <div class="btn-list">
                    ...
                    <div class="form-group">
                        <div id="materials--btn-group" class="btn-group w-100">
                            <button type="button" class="btn btn-primary" data-tab="documents">Documents</button>
                            <button type="button" class="btn" data-tab="images">Images</button>
                            <button type="button" class="btn" data-tab="miscellaneous">Miscellaneous</button>
...
<script>
    var CFG__API_REST_SPACES_FILEPATH = '/uploads/documents/';
    var _type = 'documents';
    //console.log(CFG__API_URL);
    //https://api.loopdgtl.developer.brauntech.com.br/v1/upload/images
            ...
            function callback(data){
                ...
                if (isObjEmpty(response.error)) {
                    for (var i = 0; i < response.length; i++){
                        var thumbIcon = "",
                            fileType = "",
                            fileOpenClick =   //'<a href="' + CFG__API_URL+CFG__API_VERSION + CFG__API_REST_SPACES_FILEPATH + response[i].filename + '"> '
                        ...
                        if (contentType.includes("image")) {
                            thumb = '<span class="avatar me-2" style="background-image: url(\'' + CFG__API_URL + '/d/uploads/' + _type + '/' + response[i].filename + '\');background-color: unset;min-width: 80px;"></span>';
                        } else {
                            thumb = '<span class="avatar me-2" style="background-image: url(/assets/img/icons/file-types/png/' + thumbIcon + '.png);background-color: unset;min-width: 80px;"></span>';
                        }
                        ...
                        tbodyHTML +=
                            ...
                            +              thumb
                            + '            <div class="flex-fill">'
                            + '                <div class="font-weight-medium"><a href="javascript:OpenFile(\'/d/uploads/' + _type + '/' + response[i].filename + '\', \'' + response[i].contentType + '\');" class="text-reset">' + response[i].filename + '</a></div>'
                            + '                <div class="text-muted">File <b>' + fileType + '</b></div>'
                            + '            </div>'
                            + '        </div>'
                            + '    </td>'
                            + '    <td class="text-muted" data-label="Size">'
                            + '        ' + formatBytes(response[i].length, 2)
                            + '    </td>'
                            + '    <td>'
                            + '        <div class="btn-list flex-nowrap">'
                            + '            <a href="javascript:OpenFile(\'/d/uploads/' + _type + '/' + response[i].filename + '\', \'' + response[i].contentType + '\');" class="btn btn-primary">'
                            ...
                            + '                View'
                            + '            </a>'
                            + '            <a href="javascript:window.open(\'' + CFG__API_URL + '/d/uploads/' + _type + '/' + response[i].filename + '?ContentDisposition=attachment\');" class="btn btn-primary">'
    ...
    function page__materials__deleteFile(filename){
        alertOpen({
            ...
            function(){
                xdr('DELETE', CFG__API_URL + CFG__API_VERSION + CFG__API_REST_SPACES_FILEPATH + filename,
                    null,
                    function callback(data){
