...
<div class="page page-center">
    <div class="container-tight py-4">
        ...
        <form id="frmRequestReset" name="frmRequestReset" class="card card-md">
            <div class="card-body">
                <h2 class="card-title text-center mb-4">I forgot my password</h2>
                <p class="text-muted mb-4">Enter your username to recover your password.</p>
                <div class="mb-3">
                    <label class="form-label">Username</label>
                    <input name="username" type="text" class="form-control" minlength="4" maxlength="40" placeholder="Username" autocomplete="off" autofocus />
                </div>
...
<script>
        ...
        $.ajax({
            ...
            success: function(response){
                if (response.success === true) {
                    //location.href = "<?=$ini['APP_ADMIN_URL']?>/senha/confirmar-codigo?wa=wsignin1.0&wreply=<?=$ini['APP_ADMIN_URL']?>&lc=1046&mkt=pt-br&mn=" + usernameObj.value;
                    router.navigate("/password/confirm-code?wa=wsignin1.0&wreply={{settings.site.url}}&lc=1046&mkt=pt-br&mn="+usernameObj.value);
                } else {
                    frmRequestReset.querySelector("#msg").insertAdjacentHTML('beforeBegin', 
    ...
    /**frmRequestReset.addEventListener("keydown", function(e){
        e.target.classList.remove("error");
        e.target.parentElement.classList.remove("error");
    });
    
    const frmRequestReset_inputs = document.querySelectorAll("input, select, textarea");
    frmRequestReset_inputs.forEach(input => {
